# SmithCap Financial Management Office — Master Project File
**Created:** 2026-05-24
**Owner:** David Smith
**Lead Agent:** finance-cfo
**Status:** 🟡 Active — Building Out

---

## Overview
The SmithCap Financial Management Office (FMO) is the centralized financial command structure for David Smith's full portfolio — personal finances AND all 7 business ventures. It operates as a virtual family office: managing personal wealth, tax strategy, bookkeeping, investments, compliance, and financial modeling in one unified system.

---

## Org Chart

```
David Smith (Owner)
└── finance-cfo (Chief Financial Officer)
    ├── finance-cpa (CPA — Tax, Compliance, Entity Structure)
    │   └── finance-tax-strategist (Advanced Tax Planning)
    ├── finance-advisor (Personal Financial Advisor)
    │   └── finance-investment-manager (Investment Manager)
    ├── finance-bookkeeper (Bookkeeper — all entities + personal)
    ├── finance-analyst (Financial Modeling & Investor Decks)
    └── finance-compliance (Nonprofit & Grant Compliance)
```

---

## Scope: Personal + Business

### Personal Finances (finance-advisor lead)
- Personal net worth tracking
- Personal budget & savings rate
- Debt management & SBA DTI optimization
- Retirement accounts (employer 401k + Solo 401k + Roth IRA)
- Insurance coverage (life, disability, E&O, GL)
- Wealth building roadmap (3-phase, 2026–2030)

### Business Finances (finance-cfo lead)
| Entity | Type | FMO Role |
|--------|------|----------|
| Smith Capital Properties | RE Holding (DBA→LLC) | Full financial management |
| YEPC — CR 132 | Development LLC | Pro forma, SBA modeling, grant compliance |
| The Elevation ATX | Hospitality LLC | Startup P&L, seed capital tracking |
| XFTC | 501(c)(3) Nonprofit | Full compliance, 990, grant reporting |
| PBS Foundation | 501(c)(3) Nonprofit | Compliance setup, 990 prep |
| Nutrue Apparel | POD E-Commerce | Revenue tracking, COGS, tax |
| S2T Designs | Web Agency | Invoicing, bookkeeping, contractor 1099s |

---

## FMO File Library

| File | Description | Status |
|------|-------------|--------|
| `PROJECT.md` | This file — master overview | ✅ Complete |
| `DASHBOARD.md` | Live consolidated financial dashboard | ✅ Built |
| `PERSONAL-FINANCES.md` | Full personal financial plan | ✅ Built |
| `CAPITAL-ROADMAP.md` | 3-year capital deployment plan (2026–2028) | ✅ Built |
| `ENTITY-TAX-CALENDAR.md` | All filing deadlines — every entity | ✅ Built |
| `BOOKKEEPING-SETUP.md` | Chart of accounts, bank setup, monthly close | ✅ Built |
| `SBA-READINESS.md` | SBA pre-qualification checklist + profile | ⬜ To Build |
| `INVESTMENT-POLICY.md` | Portfolio allocation model + IPS | ⬜ To Build |
| `INSURANCE-REVIEW.md` | Coverage gap analysis + quotes tracker | ⬜ To Build |
| `GRANT-FINANCE-TRACKER.md` | Active grants, restrictions, reporting | ⬜ To Build |

---

## Immediate Priorities (Q2 2026)

- [ ] **Confirm W-2 salary** *(David to provide when ready)* — anchor all personal planning
- [ ] **Pull debt balances** *(David to provide when ready)* — credit cards, vehicle, student loans
- [ ] **Q2 estimated tax payment** — due **June 16, 2026**
- [ ] **Open Solo 401(k)** — Fidelity or Vanguard, before Dec 31, 2026
- [ ] **File The Elevation LLC** — TX SOS ($300) — most urgent
- [ ] **Open Mercury bank accounts** — one per active LLC
- [ ] **Set up Wave bookkeeping** — free, connect all business accounts
- [ ] **Get E&O insurance quote** — S2T Designs client work exposure
- [ ] **Get term life insurance quote** — $500K–$1M, 20-year term

---

## Financial Snapshot — Targets

| Metric | May 2026 | Dec 2026 | Dec 2027 | Dec 2028 |
|--------|----------|----------|----------|----------|
| Personal net worth | TBD | +$25K | +$100K | +$400K |
| Emergency fund | TBD | 3 months | 6 months | 6 months |
| Investment portfolio | TBD | $5K | $25K | $75K |
| Retirement balance | TBD | $15K | $45K | $100K |
| Monthly passive income | $0 | $500 | $3K | $8K |
| LLCs formed | 0 | 3 | 4 | 5+ |
| Total entity revenue (monthly) | ~$2–5K | $5K+ | $12K+ | $25K+ |
