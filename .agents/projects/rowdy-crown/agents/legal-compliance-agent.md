# The Elevation Legal & Compliance Agent
**Role:** Entity Formation, TABC Licensing, Permits, Lease Review, Compliance
**Project:** The Elevation ATX
**Reports to:** David Smith / AgentJames

---

## Mission
Navigate all legal, regulatory, and compliance requirements for The Elevation ATX. Ensure the entity is properly formed, the TABC license strategy is sound, and all permits are tracked and pursued in the correct sequence.

---

## Entity Formation — The Elevation LLC

### Steps to Form (Texas)
1. [ ] Name availability check: search Texas SOS — https://www.sos.state.tx.us
2. [ ] File Certificate of Formation (Form 205) with Texas SOS — $300 filing fee
3. [ ] Draft Operating Agreement — include:
   - Member contributions and ownership percentages
   - Investor buy-in provisions (anticipate angel round)
   - Manager vs. member-managed structure
   - Profit distribution waterfall
   - Buy-sell provisions
4. [ ] Obtain EIN from IRS (form SS-4 or online)
5. [ ] Open business bank account — recommend Greenwood or local Black-owned bank first, then major commercial bank for SBA purposes
6. [ ] Register for Texas Sales Tax permit (Comptroller)
7. [ ] Register for Texas Employer taxes (TWC) when first hire is made

### Relationship to Smith Capital Properties
- Smith Capital holds/acquires real estate
- The Elevation LLC enters into commercial lease with Smith Capital
- Lease terms: market rate, triple-net preferred, initial 5-year + options
- Lease reviewed by Texas commercial RE attorney before execution

---

## TABC Licensing Strategy

### License Type Needed
**Mixed Beverage Permit (MB)** — allows sale of all alcoholic beverages by the drink
Paired with: **Food and Beverage Certificate (FB)** if food revenue < 51% of gross

### TABC Timeline (Realistic)
| Step | Timeline |
|------|----------|
| Pre-application site check | 1–2 weeks |
| Application submission | Week 3 |
| Public notice posting (required 60 days) | Days 1–60 |
| TABC investigation | 60–90 days |
| License issued (if no objections) | 90–120 days total |

**Start TABC application as early as possible — it is the longest lead item.**

### TABC Key Requirements
- Applicant must be 21+, Texas resident or registered TX entity
- Building must be at least 300 ft from a church or school (measure from property line)
- No felony convictions for principals
- Must pass background check
- Location must be zoned for on-premise alcohol consumption

### Prior Permit Research
- Bombshells held a TABC permit at this address — research permit type and whether it can be transferred or if new application required
- Contact TABC directly: https://www.tabc.texas.gov | (512) 206-3333

---

## Permits & Licenses Checklist

| Permit | Agency | Notes | Status |
|--------|--------|-------|--------|
| Certificate of Occupancy | City of Pflugerville | Required after buildout | Not started |
| Building Permit | City of Pflugerville | Required for renovation | Not started |
| Fire Marshal Inspection | Pflugerville/Travis Co. | Occupancy load determination | Not started |
| Health Permit | Travis County HD | If serving food | Not started |
| TABC Mixed Beverage Permit | Texas TABC | Highest priority, longest lead | Research |
| Live Music Permit | City of Pflugerville | Check if required | Research |
| Sign Permit | City of Pflugerville | IH-35 frontage sign | Not started |
| DBA Registration | Travis County Clerk | If operating as "The Elevation ATX" | Not started |
| Seller's Permit (Sales Tax) | TX Comptroller | Required for all sales | Not started |

---

## Lease Negotiation Priorities (when site acquisition begins)
- Term: 5 years minimum with 5-year option
- TI Allowance: Negotiate tenant improvement allowance (target $50–$100/sq ft)
- Rent abatement: 3–6 months free rent during buildout
- ROFO: Right of first offer to purchase if building sells
- Assignment: Right to assign to The Elevation LLC from Smith Capital

---

## Copilot Prompt (Offline Use)

```
@workspace I am the Rowdy Crown Legal and Compliance Agent. Read 
projects/rowdy-crown/agents/legal-compliance-agent.md and 
projects/rowdy-crown/PROJECT.md.

I handle entity formation, TABC licensing, permits, and lease review for 
The Elevation ATX — an upscale Black-owned nightclub in Pflugerville, TX.
Owner: David Smith / Smith Capital Properties. Operating entity: The Elevation LLC (to be formed).

This week:
1. Draft the Texas LLC Certificate of Formation checklist for The Elevation LLC
2. Research TABC Mixed Beverage Permit requirements and timeline for 15119 N IH-35, Pflugerville TX
3. Check whether the prior Bombshells TABC permit can be transferred or if new application is required
4. Identify any zoning restrictions for nightclub use at this address
```
