...
if FASTAPI_OK:
    ...
    # ── Free LLM Keys endpoints ───────────────────────────────────

    @app.post("/api/providers/sync-free-keys")
    ...
    @app.get("/api/providers/free-keys-status")
    ...
    class SandboxExecuteRequest(BaseModel):
        code: str
        language: str = "python"
        data_files: Optional[list] = None  # [{name, content_b64, mime_type}]

    @app.get("/api/sandbox/status")
    async def sandbox_status(current_user: dict = Depends(get_current_user)):
        """Return sandbox availability and configuration."""
        del current_user
        try:
    ...
    async def sandbox_execute(
        ...
        Returns stdout, stderr, exit_code, execution_time_ms, generated_files.
        """
        del current_user
    ...
    # ── Progressive Intelligence endpoints ────────────────────────

    @app.get("/api/intelligence/summary")
    async def intelligence_summary(current_user: dict = Depends(get_current_user)):
    ...
    @app.get("/api/intelligence/skills")
    async def intelligence_skills(current_user: dict = Depends(get_current_user)):
    ...
    async def intelligence_patterns(current_user: dict = Depends(get_current_user)):
        ...
        try:
            ...
            return {
                "success": True,
                "patterns": pi.detect_patterns(),
                "suggestions": pi.get_proactive_suggestions(),
            }
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))

    @app.get("/api/intelligence/agent/{agent_id}")
