...
class HubClient:
    ...
    def put_json(self, path, data=None, timeout: float = TIMEOUT) -> dict | None:
        """Public alias for _put — used by desktop UI for arbitrary PUT calls."""
        result = self._request("PUT", path, data=data, timeout=timeout)
        return result if isinstance(result, dict) else None
