...
if (-not (Test-Path $venvPath)) {
    ...
    Write-Host "  [OK] venv created at $venvPath" -ForegroundColor Green
}
...
if (Test-Path $venvActivate) {
    ...
    Write-Host "  [!]  Could not find venv activate script -- running without venv" -ForegroundColor Yellow
}
...
if (-not (Test-Path $markerFile)) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  [X] pip install failed -- check requirements.txt" -ForegroundColor Red
        exit 1
    }
} else {
