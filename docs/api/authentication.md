...
ArchonHub exposes a compact auth surface: login, self-registration (admin-gated unless no users exist), and current-user introspection. The iOS client consumes these endpoints directly through `HubClient.login(...)` and stores the JWT in the keychain.
...
curl -X POST http://localhost:8765/api/auth/login \
  ...
  -d '{"username": "admin", "password": "ArchonHub2024!"}'
```
...
- Returns a JWT and the created user record. Registration is admin-only after bootstrap unless there are no users yet.
...
curl -X POST http://localhost:8765/api/auth/register \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <jwt>' \
  -d '{"username": "admin", "email": "admin@example.com", "password": "ArchonHub2024!", "role": "string"}'
```
...
- Returns the current user object `{ id, username, email, role, ... }` without a success wrapper.
...
- Confirm `Authentication API` responses use ISO 8601 UTC timestamps.
...
- `Authentication API` is documented from the current ArchonHub source tree rather than a separate OpenAPI export.
